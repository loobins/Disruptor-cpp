...
using Sequencers = ::testing::Types< MultiProducerSequencer< int >, SingleProducerSequencer< int > >;

TYPED_TEST_CASE(SequencerTestFixture, Sequencers);

TYPED_TEST(SequencerTestFixture, ShouldStartWithInitialValue)
{
    EXPECT_EQ(this->m_sequencer->next(), 0);
}

TYPED_TEST(SequencerTestFixture, ShouldBatchClaim)
{
    EXPECT_EQ(this->m_sequencer->next(4), 3);
}

TYPED_TEST(SequencerTestFixture, ShouldIndicateHasAvailableCapacity)
{
...
TYPED_TEST(SequencerTestFixture, ShouldIndicateNoAvailableCapacity)
{
    this->m_sequencer->addGatingSequences({ this->m_gatingSequence });
...
TYPED_TEST(SequencerTestFixture, ShouldHoldUpPublisherWhenBufferIsFull)
{
    this->m_sequencer->addGatingSequences({ this->m_gatingSequence });
...
TYPED_TEST(SequencerTestFixture, ShouldThrowInsufficientCapacityExceptionWhenSequencerIsFull)
{
    this->m_sequencer->addGatingSequences({ this->m_gatingSequence });
...
TYPED_TEST(SequencerTestFixture, ShouldCalculateRemainingCapacity)
{
    this->m_sequencer->addGatingSequences({ this->m_gatingSequence });
...
TYPED_TEST(SequencerTestFixture, ShoundNotBeAvailableUntilPublished)
{
...
TYPED_TEST(SequencerTestFixture, ShouldNotifyWaitStrategyOnPublish)
{
...
TYPED_TEST(SequencerTestFixture, ShouldNotifyWaitStrategyOnPublishBatch)
{
    auto waitStrategyMock = std::make_shared< testing::NiceMock< WaitStrategyMock > >();
    auto sequencer = std::make_shared< TypeParam >(this->m_bufferSize, waitStrategyMock);
...
TYPED_TEST(SequencerTestFixture, ShouldWaitOnPublication)
{
...
TYPED_TEST(SequencerTestFixture, ShouldTryNext)
{
    this->m_sequencer->addGatingSequences({ this->m_gatingSequence });
...
TYPED_TEST(SequencerTestFixture, ShouldClaimSpecificSequence)
{
...
TYPED_TEST(SequencerTestFixture, ShouldNotAllowBulkNextLessThanZero)
{
    EXPECT_THROW(this->m_sequencer->next(-1), ArgumentException);
}

TYPED_TEST(SequencerTestFixture, ShouldNotAllowBulkNextOfZero)
{
    EXPECT_THROW(this->m_sequencer->next(0), ArgumentException);
}

TYPED_TEST(SequencerTestFixture, ShouldNotAllowBulkNextGreaterThanBufferSize)
{
    EXPECT_THROW(this->m_sequencer->next(this->m_bufferSize + 1), ArgumentException);
}

TYPED_TEST(SequencerTestFixture, ShouldNotAllowBulkTryNextLessThanZero)
{
    EXPECT_THROW(this->m_sequencer->tryNext(-1), ArgumentException);
}

TYPED_TEST(SequencerTestFixture, ShouldNotAllowBulkTryNextOfZero)
{
