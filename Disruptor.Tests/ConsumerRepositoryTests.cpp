...
{
    EXPECT_TRUE(m_consumerRepository.getBarrierFor(m_handler1) == nullptr);
}
...
TEST_F(ConsumerRepositoryTestsFixture, ShouldRetrieveEventProcessorForHandler)
{
    ...
    EXPECT_TRUE(m_consumerRepository.getEventProcessorFor(m_handler1) == m_eventProcessor1);
}
...
TEST_F(ConsumerRepositoryTestsFixture, ShouldIterateAllEventProcessors)
{
    ...
    EXPECT_EQ(seen1, true);
    EXPECT_EQ(seen2, true);
}
