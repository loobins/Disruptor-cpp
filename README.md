...
The library requires a **C++20** compiler and has **no third-party dependencies** — only the
standard library and the platform threading library (pthreads on Linux/macOS).
...
Optionally you may want to compile and run the unit tests and benchmarks. The tests are built
with [GoogleTest](https://github.com/google/googletest) (bundled in `googletest-release-1.8.0/`)
and compilation is activated by the **DISRUPTOR_BUILD_TESTS** flag:
